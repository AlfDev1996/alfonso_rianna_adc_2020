...
/**
 * Copyright 2017 Universita' degli Studi di Salerno Licensed under the Apache
 ...
 * governing permissions and limitations under the License. An auction mechanism
 * API based on P2P . Each peer can sell and buy goods using a Second-Price
 * Auctions (EBay). second-price auction is a non-truthful auction mechanism for
 * multiple items. Each bidder places a bid. The highest bidder gets the first
 * slot, the second-highest, the second slot and so on, but the highest bidder
 * pays the price bid by the second-highest bidder, the second-highest pays the
 * price bid by the third-highest, and so on.
 */
public interface AuctionMechanism {
     ...
     * @param _reserved_price a double value that is the reserve minimum pricing
     * selling.
